...
from binary_be import BE_BinaryStream as BinaryStream
import argparse
...
def readerGet(filepath, silent=False, split=False):
    ...
    # open file and create stream (BIG-ENDIAN for Dishonored UPK)
    ...
    # check UPK signature
    if sign != sign_upk:
        raise Exception(f"Signature mismatch: got 0x{sign:08X}, expected 0x{sign_upk:08X}")
