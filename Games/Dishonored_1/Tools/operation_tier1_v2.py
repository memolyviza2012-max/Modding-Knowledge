...
2. Use standard gaming terminology in Thai (e.g., "บันทึกเกม" for Save, "โหลดเกม" for Load).
...
def execute_tier_1():
    ...
    print("="*60)
    print("[*] เริ่มปฏิบัติการแปล Tier 1: UI & Menus")
    print("[*] ระบบ True Auto-Resume พร้อมทำงาน")
    ...
    for filename in TARGET_FILES:
        ...
        if not os.path.exists(source_path):
            print("[!] ไม่พบไฟล์ {0} ในคลังต้นฉบับ ข้ามการทำงาน...".format(filename))
            continue
        ...
        if os.path.exists(target_path):
            ...
            if start_line >= len(source_lines):
                ...
                print("    >> กำลังเชื่อมต่อระบบ... แปลต่อจากบรรทัดล่าสุดทันที")
                file_mode = 'a'
        else:
