...
def run_fresh_translation():
    print("="*60)
    print("MATRIX REPORT - เมทริกซ์รายงาน")
    print("="*60)
    print("[*] ปฏิบัติการ: Clean & Re-Translate (V.1.3 - Newline Patch)")
    print("[*] กำลังล้างข้อมูลเก่าใน: {0}".format(TARGET_DIR))
    ...
    if os.path.exists(TARGET_DIR):
        ...
        for filename in os.listdir(TARGET_DIR):
            ...
            try:
                ...
                print("[!] ไม่สามารถลบ {0} ได้: {1}".format(file_path, e))
        print("[*] ลบไฟล์เก่า {0} รายการ".format(count))
    else:
        os.makedirs(TARGET_DIR)

    print("[*] เริ่มปฏิบัติการแปลใหม่ด้วย Smart Pacing + Newline Patch")
