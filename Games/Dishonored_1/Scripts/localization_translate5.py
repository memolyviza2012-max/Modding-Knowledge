# ==========================================
# ไฟล์: localization_translate5.py (google.genai)
# แปล 5 ไฟล์แรกใน 02_translated/Localization/INT/
# ==========================================
import sys
...
def find_eng_lines_simple(parsed):
    result = []
    for (i, k, v, raw) in parsed:
        if re.search(r'[a-zA-Z]{2,}', v) and not has_thai(v):
            result.append((i, k, v, raw))
    return result

def find_eng_lines_to_translate(target_parsed, ref_dict):
    result = []
    for (i, k, v, raw) in target_parsed:
        ref_v = ref_dict.get(k, "")
        # Only translate if target is English (no Thai) AND ref is English
        if re.search(r'[a-zA-Z]{2,}', v) and not has_thai(v) and re.search(r'[a-zA-Z]{2,}', ref_v):
            result.append((i, k, v, raw))
    return result
...
def deduplicate(eng_lines):
    ...
    unique = []
    dup_map = {}
    for (i, k, v, raw) in eng_lines:
        if v not in seen:
            seen[v] = k
            unique.append((i, k, v, raw))
        else:
            dup_map[k] = seen[v]
...
def translate_file(target_path, ref_path):
    ...
    ref_dict = {k: v for (i, k, v, raw) in ref_parsed}
    target_keys = {k for (i, k, v, raw) in target_parsed}
    ref_keys = set(ref_dict.keys())

    missing = ref_keys - target_keys
    extra = target_keys - ref_keys
    if missing:
        safe_print(f" [!] Missing keys: {sorted(list(missing))[:5]}")
    if extra:
        safe_print(f" [!] Extra keys: {sorted(list(extra))[:5]}")

    eng_lines = find_eng_lines_to_translate(target_parsed, ref_dict)
    unique_eng, dup_map = deduplicate(eng_lines)

    if dup_map:
        safe_print(f" [i] {len(dup_map)} duplicates (reuse)")
    if not unique_eng:
        safe_print(f" [=] No English to translate")
        return 0

    safe_print(f" [i] {len(unique_eng)} unique lines ({len(eng_lines)} total)")

    lines_for_api = [f"{k}={ref_dict.get(k, v)}" for (i, k, v, raw) in unique_eng]
    res = translate_batch(lines_for_api)
    if not res:
        safe_print(f" [X] Translation failed")
        return 0
    ...
    if len(trans_dict) != len(lines_for_api):
        safe_print(f" [!] Sent {len(lines_for_api)}, got {len(trans_dict)}")
...
def find_files():
    ...
    for path in glob.glob(SOURCE_DIR + "\\*.int"):
        ...
        try:
            ...
            eng = find_eng_lines_simple(parsed)
            if eng:
                results.append((os.path.basename(path), len(eng), path))
        except:
            continue
    return sorted(results, key=lambda x: x[1])

def main():
    safe_print("=== Localization Translate 5 files ===\n")
    files = find_files()
    safe_print(f"[i] Files with English: {len(files)}\n")

    if not files:
        safe_print("[X] No files")
        return
    ...
    for idx, (fname, eng_count, target_path) in enumerate(files[:MAX_FILES]):
        ...
        safe_print(f"\n=== [{idx+1}/{MAX_FILES}] {fname} ({eng_count}) ===")
        count = translate_file(target_path, ref_path)
        total += count
        state["files_done"] += 1
        state["lines_done"] = total
