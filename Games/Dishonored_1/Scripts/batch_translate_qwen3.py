# ==========================================
# ไฟล์: batch_translate_qwen3.py (Qwen3 V1.1 - LM Studio Optimized)
# ==========================================
import sys
...
# [V1.1] timeout=600.0 (10 นาที) — ป้องกันสคริปต์ตัดสายทิ้งตอนการ์ดจอประมวลผลนาน
client = OpenAI(base_url=LM_STUDIO_URL, api_key="lm-studio", timeout=600.0)

BATCH_SIZE = 50   # ลดลง — local model context สั้นกว่า Gemini
MAX_WORKERS = 1   # local model รันได้ทีละ request เดียว
RETRY_MAX = 3
RETRY_BASE_DELAY = 2
...
def translate_batch(lines):
    prompt = "".join(lines)
    # [V1.1] ดัก Local Model ไม่ให้มโนหรือชวนคุยนอกเรื่อง
...
def sync_translate(int_data, th_data, th_yaml_path):
    ...
    safe_print(f" [→] {total} missing | {n_batches} batches")
...
def main():
    safe_print("=== Dishonored TH Localization — Qwen3 V1.1 (LM Studio) ===\n")
    safe_print(f"Model : {LM_STUDIO_MODEL}")
    safe_print(f"Server: {LM_STUDIO_URL}\n")
