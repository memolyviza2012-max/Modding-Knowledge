# -*- coding: utf-8 -*-
"""
run_translation_SHDW.py
========================
สคริปต์แปลภาษาอัตโนมัติสำหรับเกม Space Hulk: Deathwing ด้วย DeepSeek API
ดัดแปลงจากระบบของ GOTG เปลี่ยนมารองรับไฟล์ CSV (pylocres) โครงสร้าง utf-16-le

[ฟีเจอร์เด็ดระดับโปร]:
  1. CSV Row Tracking: อาศัย Index ของแถวใน CSV แทนคีย์ เพื่อจัดการไฟล์ CSV ได้ตรงจุด
  2. Explicit Tag Masking: ถอดแท็กเกม (<color>, \n, %s) ออกก่อนส่งให้ AI
  3. Full-string is_thai() Scan: ตรวจสอบภาษาไทยทั้งประโยค
  4. Atomic Checkpoint Save: เซฟไฟล์ลง .tmp ก่อนทำการ os.replace()
"""
...
# ไฟล์เป้าหมาย (CSV ที่ export ออกมาจาก pylocres)
CSV_FILEPATH = r"E:\Mod_Workspace\SpaceHulkDeathwing_Mod_Worksace\Translation\SHDW_th.csv"

# คำหยาบ Canary ตรวจจับ AI หลอน
CANARY_WORDS = ["กีวางไร่อะ", "หลอนยา", "มั่วซั่ว"]

# ==============================================================================
# [ส่วนที่ 2: Master Prompt ระดับเซียนสำหรับจักรวาล Warhammer 40k]
# ==============================================================================
SYSTEM_PROMPT = """I want you to act as a Master-Level English-to-Thai Video Game Localization Specialist. Your assignment is "Space Hulk: Deathwing" — a grimdark, tactical first-person shooter set in the Warhammer 40,000 universe.

=== 1. TONE & CONTEXTUAL RULES ===
- Atmosphere: Grimdark, religious zealotry, oppressive, militaristic, and terrifying.
- Swearing & Exclamations: Space Marines NEVER use common casual swearing (like ไอ้เหี้ย, ไอ้สัส). Use terms fitting religious warrior-monks. For insults use "สวะ" (Scum), "พวกนอกรีต" (Heretic), "สวะเอเลี่ยน" or "ซีนอส" (Xenos), "เดรัจฉาน" or "อสุรกาย" (Abomination/Beast). For exclamations use "แด่องค์จักรพรรดิ!" (By the Emperor!), "ขอองค์จักรพรรดิคุ้มครอง" (Emperor Protects), or "จงสารภาพบาปซะ!" (Repent!).
- Dialogue & Lore: Speak like fanatical, genetically modified super-soldiers (Space Marines of the Dark Angels chapter). Formal, authoritative, and zealous.
- Pronouns: 
  * Space Marines (Brothers): Formal and respectful to each other (ข้า, ท่าน, พี่น้อง).
  * Enemies (Genestealers): Referred to with disgust (พวกมัน, เดรัจฉาน, อสุรกาย).
...
- Space Marine = สเปซมารีน
- Space Marines = สเปซมารีน
- Genestealer = จีนสตีลเลอร์
- Genestealers = จีนสตีลเลอร์
- Deathwing = เดธวิง
- Dark Angels = ดาร์กแองเจิล
- Emperor = องค์จักรพรรดิ
- Space Hulk = สเปซฮัลค์
- Librarian = ไลบราเรียน
- Apothecary = อะพอธเธคารี
- Terminator = เทอร์มิเนเตอร์
- Terminators = เทอร์มิเนเตอร์
- Tyranid = ไทรานิด
- Tyranids = ไทรานิด
- Assault Cannon = แอสซอลต์แคนนอน
- Storm Bolter = สตอร์มโบลเตอร์
- Power Sword = พาวเวอร์ซอร์ด
- Force Sword = ฟอร์ซซอร์ด
- Plasma Cannon = พลาสม่าแคนนอน
- Heavy Flamer = เฮฟวี่เฟลมเมอร์
- Chapter Master = แชปเตอร์มาสเตอร์
- Primarch = ไพรมาค
- Lion El'Jonson = ไลออน เอล'จอนสัน
- Brother = พี่น้อง
- Brother-Captain = บราเธอร์-กัปตัน
- Belial = เบลิอัล
- Caliban = คาลิบาน
- Psyker = ไซเกอร์
- Warp = วาร์ป
...
def save_csv_checkpoint(headers, rows, filepath):
    ...
    try:
        ...
        with open(tmp_file, 'w', encoding='utf-16-le', newline='') as f:
            writer = csv.writer(f)
...
def main():
    print("=" * 60)
    print("      Space Hulk: Deathwing - Ultimate Translation Engine")
    print("=" * 60)
    ...
    try:
        with open(CSV_FILEPATH, 'r', encoding='utf-16-le', newline='') as f:
            reader = csv.reader(f)
            try:
            ...
            for row in reader:
                # ทำให้แน่ใจว่าแถวมีอย่างน้อย 5 คอลัมน์ (ถึงคอลัมน์ translation)
                while len(row) < 5:
                    row.append("")
                csv_rows.append(row)
    except Exception as e:
    ...
    if not pending_tasks:
        print("      ยินดีด้วยครับบอส! เกมนี้ได้รับการแปลไทยครบ 100% เรียบร้อยแล้ว!")
        return
    ...
    for idx, batch in enumerate(batches, 1):
        ...
        for task in batch:
            ...
            if tid in api_results:
                ...
                # นำคำแปลกลับไปใส่ใน row[4]
                row_idx = task["row_idx"]
                csv_rows[row_idx][4] = final_thai
                translated_count += 1
            else:
